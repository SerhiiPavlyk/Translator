...
void BeginASMFile(FILE* fout)					//"head" asm file
{
	fprintf(fout, ".686\n.model flat, stdcall\noption casemap :none\n");
...
void BeginCodeSegm(FILE* fout)					//start code
{
	fprintf(fout, "\n.CODE\n");
	fprintf(fout, "start:\n");
...
void CheckPresent()								//Determination of the presence of Keyword
{
...
void PrintData(FILE* f)							//segment of data
{
	fputs("\n;-------------------------------------------------------------------------", f);
	fputs("\n;------User's data--------------------------------------------------------", f);
	fputs("\n;-------------------------------------------------------------------------\n", f);
...
void PrintAND(FILE* f)							//Procedure "And"
{
	fputs("\n;-------------------------------------------------------------------------", f);
	fputs("\n;------Procedure \"And\"--------------------------------------------------", f);
	fputs("\n;-------------------------------------------------------------------------\n", f);
...
void PrintOR(FILE* f)							//Procedure "Or"
{
	fputs("\n;-------------------------------------------------------------------------", f);
	fputs("\n;------Procedure \"Or\"---------------------------------------------------", f);
	fputs("\n;-------------------------------------------------------------------------\n", f);
...
void PrintNOT(FILE* f)							//Procedure "Not"
{
	fputs("\n;-------------------------------------------------------------------------", f);
	fputs("\n;------Procedure \"Not\"--------------------------------------------------", f);
	fputs("\n;-------------------------------------------------------------------------\n", f);
...
void PrintEQ(FILE* f)							//Procedure "Equal"
{
	fputs("\n;-------------------------------------------------------------------------", f);
	fputs("\n;------Procedure \"Equal\"------------------------------------------------", f);
	fputs("\n;-------------------------------------------------------------------------\n", f);
...
void PrintGE(FILE* f)							//Procedure "Greate"
{
	fputs("\n;-------------------------------------------------------------------------", f);
	fputs("\n;------Procedure \"Greate\"-----------------------------------------------", f);
	fputs("\n;-------------------------------------------------------------------------\n", f);
...
void PrintLE(FILE* f)							//Procedure "Less"
{
	fputs("\n;-------------------------------------------------------------------------", f);
	fputs("\n;------Procedure \"Less\"-------------------------------------------------", f);
	fputs("\n;-------------------------------------------------------------------------\n", f);
...
void PrintCheck(FILE* f)						//Procedure "Check zero"
{
	fputs("\n;-------------------------------------------------------------------------", f);
	fputs("\n;------Procedure \"Check zero\"-------------------------------------------", f);
	fputs("\n;-------------------------------------------------------------------------\n", f);
	fputs("Check_ PROC\n", f);
	fputs("\tpop edx\n", f);
	fputs("\tpop eax\n", f);
...
void PrintInput(FILE* f)						//Procedure "Input"
{
	fputs("\n;-------------------------------------------------------------------------", f);
	fputs("\n;------Procedure \"Input\"------------------------------------------------", f);
	fputs("\n;-------------------------------------------------------------------------\n", f);
...
void PrintOutput(FILE* f)						//Procedure "Output"
{
	fputs("\n;-------------------------------------------------------------------------", f);
	fputs("\n;------Procedure \"Output\"-----------------------------------------------", f);
	fputs("\n;-------------------------------------------------------------------------\n", f);
...
void PrintEnding(FILE* f)						//"end" asm file
{
	fputs("exit_label:\n", f);
	fputs("invoke WriteConsoleA,hConsoleOutput,ADDR msg1310,SIZEOF msg1310,ADDR NumberOfCharsWrite,0\n", f);
...
bool Prioritet(TypeOfLex t, StackType s)		//put priorities of operations
{
	bool r;
	r = (
...
int ConvertToPostfixForm(int i)					//Forms in the array a sequence of lexeme numbers that corresponds to the postfix form
{
	Stack.Init(&Stack.S);
	int n, z;
	n = 0;
	for (; ((Data.TableLexems[i + n].type != LSeparator) && (Data.TableLexems[i + n].type != LBodyIf)); ++n);
	//Setting the correctness and length of the input array
	int k;
	k = i + n;
	for (z = 0; i < k; ++i)
	{
...
void GenASMCode(const char* str, FILE* f)			//assembly code generation
{
	int n;
...
void PrintCode(FILE* f)								//print assembly code to output file
{
	int lab = 0;
	int ifLabelIndex = 0;
	Lexema l;										//current lexeme
	int i = 0;
	StackIf.Init(&StackIf.S);
	forStack.Init(&forStack.S);

	do												//searching code begining
	{
		++i;
	...
	for (;; ++i)
	{
		...
		if ((l.type == LEnd) && (Data.TableLexems[i + 1].type != LEOF))
		{
			if (startStack.S.top >= 0)
			{
				...
				if (temp1 == 1) fprintf(f, "\tdec eax\n");
				else fprintf(f, "\tinc eax\n");
		...
		if (l.type == LFor)
		{
			...
			i += 3;
			bufi = i;
			i = ConvertToPostfixForm(i + 1);						//postfix expression generation			
			if (i < 0)
			{
				i = -i;
				puts("IE\n");
			...
			//generation of assembly code from a postfix expression
			...
			i = ConvertToPostfixForm(i + 1);						//generation of the postfix expression
			inner_for_flag = 1;
			GenASMCode(Data.TableLexems[bufi - 1].name, f);
			inner_for_flag = 0;
...
void GenerateCode(FILE* f)											//generate code
{
	BeginASMFile(f);
	CheckPresent();
	PrintData(f);
